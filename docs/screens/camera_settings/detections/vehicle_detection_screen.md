...
| VEHICLE-007 | Refresh preview button | `RefreshPreviewButton` (shared widget, `lib/widgets/refresh_preview_button.dart`) | re-renders the preview (simulated ~600ms delay with a spinner in place of the icon); no real camera snapshot fetch exists yet; same widget used by [privacy_mode_screen.md](../video_display/privacy_mode_screen.md) (PRIV-005) and every other preview screen |
