...
class NuraeyeClient {
  ...
  static final Map<String, Map<String, dynamic>> _capabilitiesCacheByHost = {};
  ...
  /// `LiveViewController`'s `GetWebRtcUri`, or `camera_settings_cache.dart`'s `prefetchAndCache`
  /// firing ~9 clients at once), each built as a fresh, short-lived `NuraeyeClient`. Without
  ...
  static void debugClearCaches() {
    ...
    _capabilitiesCacheByHost.clear();
  ...
  Future<CameraResult<Map<String, dynamic>>> _getNightVisionType(Duration timeout) async {
    ...
    final capsResult = await _capabilities(timeout);
  ...
  Future<CameraResult<Map<String, dynamic>>> _capabilities(Duration timeout) async {
    final cached = _capabilitiesCacheByHost[connection.host];
    if (cached != null) return CameraSuccess(cached);

    final result = await _get('/nuraeye/capabilities', timeout);
    if (result case CameraSuccess<Map<String, dynamic>>(:final value)) {
      _capabilitiesCacheByHost[connection.host] = value;
    }
    return result;
  }
