...
void main() {
  // These tests share one CameraConnection.host across the whole file; without this, the
  // process-lifetime endpoint/options cache MaskClient now keeps (per direct user requirement
  // that it survive across screen visits, not just one client instance) would leak state
  // between otherwise-independent test cases.
  setUp(MaskClient.debugClearCaches);
  ...
  test('getMaskOptions is only fetched once unless forceRefresh is passed', () async {
    ...
    await client.getMaskOptions();
    await client.getMaskOptions();
    expect(getMaskOptionsCount, 1);

    await client.getMaskOptions(forceRefresh: true);
