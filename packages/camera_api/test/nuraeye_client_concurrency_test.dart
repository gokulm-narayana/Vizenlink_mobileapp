...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final nuraeye = NuraeyeClient(
        ...
        httpClient: mockNuraeyeRest(
          routeByPath({
            ...
            '/nuraeye/webrtc-uri': (request) => jsonOk({'port': 8444, 'url': 'http://192.168.1.50:8444/webrtc'}),
          }),
          onLogin: () => loginCalls++,
        ),
      );
      ...
      // live_view_screen.dart's GetVideoMode racing LiveViewController's GetWebRtcUri on
      // screen open, both starting with an empty token cache.
      final results = await Future.wait([
        nuraeye.call('GetVideoMode'),
        nuraeye.call('GetWebRtcUri', params: {'profile_token': 'Profile_2'}),
      ]);
