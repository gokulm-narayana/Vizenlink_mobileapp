...
void main() {
  ...
  test(
    'getSerialNumber parses SerialNumber from a real-shaped GetDeviceInformationResponse',
    () async {
  ...
  test(
    'getSerialNumber surfaces CameraFailure for a rejected (HTTP 401) request',
    () async {
      final client = OnvifDeviceClient(
        const CameraConnection(
          host: '192.168.1.50',
  ...
  test(
    'getDeviceIdentity parses name and location from GetScopesResponse, URI-decoded',
    () async {
      final client = OnvifDeviceClient(
        const CameraConnection(
          host: '192.168.1.50',
  ...
  test('setDeviceName sends exactly one URI-encoded name Scopes entry', () async {
    final client = OnvifDeviceClient(
      ...
      httpClient: MockClient((request) async {
        ...
        expect(
          request.body,
          contains('onvif://www.onvif.org/name/Back%20Yard'),
        );
  ...
  test(
    ...
    () async {
      ...
      expect((result as CameraFailure<void>).reason, 'Sender not authorized');
    },
  );

  test(
    'getSystemDateAndTime parses TZ, DST, and UTC date/time from the response',
    () async {
      final client = OnvifDeviceClient(
        const CameraConnection(
          host: '192.168.1.50',
  ...
  test(
    'setTimeZone re-reads the current UTC clock first, then echoes it back with the new TZ',
    () async {
      var requestCount = 0;
      final client = OnvifDeviceClient(
        const CameraConnection(
  ...
  test(
    'getDeviceInformation parses all five fields from GetDeviceInformationResponse',
    () async {
      final client = OnvifDeviceClient(
        const CameraConnection(
          host: '192.168.1.50',
  ...
  test(
    'getNetworkInterfaceInfo parses interface name/MAC/IP and reports wireless correctly',
    () async {
      final client = OnvifDeviceClient(
        const CameraConnection(
          host: '192.168.1.50',
  ...
  test(
    'setUserPassword sends SetUser with username, password, and UserLevel Administrator',
    () async {
      final client = OnvifDeviceClient(
        const CameraConnection(
          host: '192.168.1.50',
  ...
  test(
    'setUserPassword XML-escapes special characters in the password',
    () async {
      final client = OnvifDeviceClient(
        const CameraConnection(
          host: '192.168.1.50',
        ...
        httpClient: MockClient((request) async {
          expect(
            request.body,
            contains('<tt:Password>a&amp;b&lt;c&gt;d</tt:Password>'),
          );
          return http.Response(
            '<?xml version="1.0" encoding="UTF-8"?>'
            '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope">'
